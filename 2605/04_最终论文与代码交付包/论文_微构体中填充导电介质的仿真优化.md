...
图 4 导通概率随介质 A 体积分数的上升呈典型渗流跃变；带误差棒的标记为问题二点名的四档（Wilson 95% 区间），星号为问题三求得的使 $P\ge90\%$ 的最低体积分数
...
图 5 $P=90\%$ 的可行边界（红线）与等成本线（白线）在 $N_B=0$ 处相切：给定价格下掺入介质 B 并不划算
...
图 6 两图同用 0–1 量程：判据阈值 δ 在 ±50% 内只挪动导通概率 0.048，而方向分布这一建模口径的影响达 0.248
