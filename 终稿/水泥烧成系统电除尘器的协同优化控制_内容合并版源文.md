...
截距不惩罚。在线性岭与二次岭各自的候选正则强度中，按训练期顺序设置3个扩展窗口滚动验证，二次岭选得$\lambda=0.1$，线性岭选得$\lambda=10$。最终模型只用训练期拟合，验证期用于模型比较与误差保护，回顾性测试期不参与选择。验证集绝对误差的90%分位数为11.78 kW，故每个候选同时报告点预测$\widehat P$与附加验证期误差修正后的功耗$\widehat P+11.78$ kW；该修正量不改变候选排序，但避免把点预测当作无误差预算。验证期误差附加量用于表示功率预测误差的量级，不保证在后续日期仍具有90%的覆盖率。
...
**图9　回顾性测试残差诊断。** 平均偏差为-10.78 kW，高功率区负偏差有所扩大；验证绝对误差90%分位数11.78 kW被用作验证期误差附加量。上线前仍需用全新日期重新校准，不能把当前测试称为真正盲测。
...
这些参数是支持域内的中心情景最佳采样可行解，不是设备安全设定。将其与同一功耗模型在各工况“历史中位控制点”上的预测比较，辅助节能幅度为9.80%～12.43%；该比较比“推荐点模型预测对历史实际均值”更同口径，但仍受排放情景假设影响。附加验证期误差修正后，表中每个功耗均再加11.78 kW。现场应用必须继续受二次电流、火花率、联锁和设定变化速率约束。

**表5.5　推荐解的历史支持域边界接近度审计**
...
数据表明：工况2的10 mg解及工况4的5 mg解已高度贴近历史支持域的马氏距离上界（$d_M^2/q_{0.975} > 90\%$）。这说明部分推荐控制点是在功耗模型与支持域边界的共同作用下推至边界附近，实际工业应用时仍需通过现场阶跃试验重新标定，不可直接将支持域边界解当作绝对安全解。
...
在线实施采用TARGET-10、TARGET-5与FALLBACK三态。排放目标由上级指令或管理规定选择，不设自动切换阈值——因为本文灰箱模型未经现场标定，不具备在线自动判定排放状态的精度。TARGET-10采用表5中10 mg/Nm³情景参数，TARGET-5采用表6中5 mg/Nm³情景参数。控制量按斜坡接近目标，试运行情景暂取电压不超过0.5 kV/min、振打周期不超过5 s/min，但现场DCS、联锁或设备制造商给出的更严限制具有优先权。四场振打先按$\rho=1$同相上界核验，再以$\rho=0.80$和0.65评估错相收益。FALLBACK在以下任一事件时触发：传感器失效、火花率越限、工况超出支持域、联锁触发或实际响应与模型预测明显不一致，此时回到现场确认的安全设定。
...
加上11.78 kW验证期误差附加量后，两档加权功耗分别为1621.19和1841.57 kW；由于两者使用同一附加量，该比例不作为严格上界，而只用于功耗预算。


![图14 分工况增耗](figures_paper/18_condition_energy_penalty.png)


**图14　四工况从10收紧至5 mg/Nm³的中心情景增耗。** 分工况为13.34%～14.11%，工况加权点估计约13.7%；该点值必须与灰箱参数组合情景范围同时报告。
...
总体尺度扫描仍保留27组：电压效应尺度$\eta\in\{0.8,1.0,1.2\}$、振打峰值尺度$b_0\in\{0.9,1.2,1.5\}$、安全裕度$s\in\{0,0.08,0.15\}$。其中26组全工况可行，增耗为10.40%～18.45%。更重要的是，灰箱参数组合扫描涵盖出口缩放0.08～0.12、三种场权重形状、三个峰值指数、三个相位尺度和三个参考周期负荷指数，共405组且全部可行；灰箱参数组合情景增耗为11.37%～14.31%，中位数12.99%。两类范围都是先验网格上的灰箱参数组合情景范围，不是统计置信区间。


![图17 增耗敏感性分布](figures_paper/16_sensitivity_distribution.png)


**图17　405组灰箱参数组合情景的增耗分布。** 出口缩放与振打相位会系统改变增耗，中心点约13.7%应与11.37%～14.31%的灰箱参数组合情景范围一起解释。
...
稳健性证据分四层：轮廓系数支持四工况分层；基线与滚动验证支持功耗模型选择；五种子与局部细化支持数值搜索稳定；灰箱参数组合情景与场权重消融检验灰箱结论对建模选择的依赖。中心增耗约13.7%，但灰箱参数组合情景中位数为12.99%，二者差异正说明不能只报告单点。功耗误差保护、测试偏差和情景范围也不能把灰箱模型变成实证模型，现场补采与盲测仍是闭环前置条件。
...
第一，设置可识别性门禁，避免用复杂算法拟合封顶噪声。第二，工况聚类不使用出口浓度和总功率，减少以结果反向定义工况的偏差。第三，功耗模型采用均值、线性、二次基线和滚动验证，并报告偏差与误差保护。第四，支持域使用逐变量分位数和经验Mahalanobis阈值，降低不合理外推。第五，优化使用五种子与局部细化，且不冒称全局最优。第六，405组灰箱参数组合情景、场权重消融和振打相位尺度把关键先验显式化。
...
1. 附件出口浓度在50 mg/Nm³附近显著截顶，无法从数据实证识别控制量对真实排放及逐次振打峰值的影响。灰箱模型只提供条件式方向与试验假设；同相振打是情景上界，错相收益需由事件级数据标定。
2. 基于入口温度、入口浓度和流量识别出4类典型工况，轮廓系数0.4084。滚动验证选择的二次岭优于均值与线性基线，验证$R^2=0.9837$、RMSE=7.12 kW；验证期误差附加量为11.78 kW。
...
5. 排放目标由10收紧至5 mg/Nm³时，中心情景加权功耗由1609.4增至1829.8 kW，增幅约13.7%；405组灰箱参数组合情景范围为11.37%～14.31%，中位数12.99%。上述数值均为情景推演，必须在补采未封顶秒级排放、振打事件和电气状态后重新标定。
