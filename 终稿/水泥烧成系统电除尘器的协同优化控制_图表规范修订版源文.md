...
本文采用的数据驱动层与灰箱情景层协同框架如图3所示。工况和功耗由实测数据辨识，排放约束由显式先验建立，两者仅在支持域优化阶段合并。
...
功耗模型的回顾性测试结果如图8所示。二次岭回归保持较高预测一致性，并优于均值预测与线性岭基线；左图虚线表示理想预测，右图RMSE采用对数纵轴以同时呈现三种模型的量级差异。
...
回顾性测试残差如图9所示。平均偏差为-10.78 kW，高功率区负偏差有所扩大；验证绝对误差90%分位数11.78 kW被用作验证期误差附加量。上线前仍需利用全新日期重新校准，不能把当前测试称为真正盲测。
...
各工况典型状态下的局部电压—功耗响应如图10所示。曲线仅在历史电压附近绘制，表明在灰箱情景下，提高电压以满足更严格的排放约束会带来能耗代价。
...
四类工况在10和5 mg/Nm³限值下的中心情景最佳采样功耗如图11所示。更严格的排放限值在全部工况下均提高功耗。
...
两档排放限值下的四电场中心情景电压如图12所示。严格约束主要抬升前两电场，同时$U_4$在部分工况承担明显补偿，因此不能将策略绝对化为“只调前级”。
...
两档排放限值下的中心情景最佳采样振打周期如图13所示。周期变化不像电压那样单向，而是在积灰损失与再飞扬峰值之间重新平衡；前级短、后级长的顺序保持不变。
...
四类工况从10收紧至5 mg/Nm³的中心情景增耗如图14所示。分工况增耗为13.34%～14.11%，工况加权点估计约13.7%；该点值必须与灰箱参数组合情景范围同时报告。
...
中心情景最佳采样解的连续排放基值与振打峰值增量如图15所示。图中虚线表示情景限值，所有柱形均为模型推演结果，而非实测排放数据。
...
五种子离散性与局部细化收益如图16所示。左图以各组合五种子中位数为基准，右图给出局部细化相对初始随机库的改善；数值稳定不等价于获得全局最优解。
...
405组灰箱参数组合情景的增耗分布如图17所示。出口缩放与振打相位会系统改变增耗，中心点约13.7%应与11.37%～14.31%的灰箱参数组合情景范围结合解释。
...
电场贡献权重消融结果如图18所示。在等权、弱前级和中心前级三种设定下，前两电场仍承担70.8%～73.7%的正向电压增量；该结果支持有条件的“前级优先”，同时说明$U_4$补偿不可忽略。
