...
针对水泥烧成系统四电场电除尘器的节能与超低排放协同控制问题，本文审计7天、10080条分钟级记录。出口浓度有效值全部位于48.74～50.00 mg/Nm³，且未覆盖10或5 mg/Nm³目标区间，故附件只能支持工况和功耗辨识，不能直接识别目标附近的控制—排放响应。本文构建 **“工况聚类—数据驱动功耗—工程灰箱排放—历史支持域优化”** 框架，以滚动验证选择二次岭功耗模型，以Deutsch–Anderson关系、积灰损失和振打再飞扬代理构造排放情景，并采用5个随机种子和局部细化搜索可行解。功耗模型验证集 **R²为0.9837、RMSE为7.12 kW** ，回顾性测试R²为0.9571。在出口记录按0.1缩放、采用中心灰箱参数且控制量位于历史支持域的条件下，10和5 mg/Nm³对应的工况加权功率分别为1609.4和1829.8 kW， **条件式增幅约13.7%** 。405组灰箱参数组合情景均可行，增耗为 **11.37%～14.31%** ；场权重消融后，前两电场承担 **70.8%～73.7%** 的正向电压增量。所有排放数值均为条件式推演，不作为附件直接识别的实证结论。
...
为识别功耗预测主要依赖哪些变量，将二次岭模型的标准化系数按绝对值排序，如图9所示。电压、电压平方项及跨电场交互项提供了主要预测信息；由于各变量之间存在相关性，系数大小只用于解释模型的预测结构，**不能直接解释为单变量因果效应**。

![图9 功耗模型标准化系数](figures_paper/08b_feature_importance.png)
...
![图10 功耗残差诊断](figures_paper/08_power_residuals.png)


图10　回顾性测试残差诊断


各工况典型状态下的局部电压—功耗响应如图11所示。曲线仅在历史电压附近绘制，表明在灰箱情景下，提高电压以满足更严格的排放约束会带来能耗代价。

![图11 电压—功耗局部响应](figures_paper/09_voltage_power_response.png)
...
四类工况在10和5 mg/Nm³限值下的中心情景最佳采样功耗如图13所示。更严格的排放限值在全部工况下均提高功耗。

![图13 两档限值的最优功耗](figures_paper/10_optimal_power.png)
...
四类工况从10收紧至5 mg/Nm³的中心情景增耗如图17所示。分工况增耗为13.34%～14.11%，工况加权点估计约13.7%；该点值必须与灰箱参数组合情景范围同时报告。

![图17 分工况增耗](figures_paper/18_condition_energy_penalty.png)
...
405组灰箱参数组合情景的增耗分布如图20所示。出口缩放与振打相位会系统改变增耗，中心点约13.7%应与11.37%～14.31%的灰箱参数组合情景范围结合解释。

![图20 增耗敏感性分布](figures_paper/16_sensitivity_distribution.png)
...
**支撑材料清单：** 原始处理数据包；results目录下功耗基线、滚动验证、中心控制、多种子稳定性、结构情景、场权重消融、动态控制表和运行摘要；figures_paper目录下21幅PNG预览、21幅PDF矢量图及figure_manifest.csv；src目录下4个完整源程序；integrity目录下确定性核验结果；revision_round1目录下修订补丁、应用报告、修改追踪表与逐点回复。
...
CAPTIONS = {
    ...
    "08b_feature_importance": "二次岭功耗模型的标准化系数显示，电压及其二次交互项是主要预测信息来源。系数反映相关性和预测贡献，不解释为单变量因果效应。",
...
TRACE_META = {
    ...
    "10_optimal_power": (str(RESULTS / "optimal_controls_central_scenario.csv"), "fig10_power", "§5.4；图13", "功耗来自数据模型；可行性来自未辨识灰箱情景"),
