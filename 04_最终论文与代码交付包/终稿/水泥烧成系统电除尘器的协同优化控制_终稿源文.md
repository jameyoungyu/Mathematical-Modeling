...
针对水泥烧成系统四电场电除尘器的节能与超低排放协同控制问题，本文审计题给7天、10080条分钟级记录。出口浓度有效值全部位于48.74～50.00 mg/Nm³，且无样本落入10或5 mg/Nm³目标区间，故附件只能支持工况与功耗辨识，不能直接识别目标附近的控制—排放响应。为此构建“工况聚类—数据驱动功耗—工程灰箱排放—历史支持域优化”框架，以滚动验证选择二次岭功耗模型，以Deutsch–Anderson关系、积灰损失和振打再飞扬代理构造显式情景，并用5个随机种子和局部细化搜索支持域内最佳采样可行解。功耗模型验证集R²为0.9837、RMSE为7.12 kW，回顾性测试R²为0.9571；验证绝对误差90%分位数11.78 kW作为保守附加量。中心情景下，10与5 mg/Nm³的工况加权功耗分别为1609.4和1829.8 kW，增幅约13.7%。405组结构情景均可行，增耗为11.37%～14.31%（中位数12.99%，非统计置信区间）；五种子最大变异系数0.181%。场权重消融后，前两电场仍承担70.8%～73.7%的正向电压增量。所有排放数值均为条件式推演，不作为附件直接识别的实证结论。
...
**AI辅助内容标注：** 本文以参赛队的人工分析、判断和核验为主。题意理解、模型选择、公式推导、参数取值、结果解释与论文定稿均由参赛队作出最终决定；OpenAI Codex仅对少量代码排错、图表版式检查和文字校对提供辅助[14]。AI输出不直接作为模型假设、参数或结论使用，具体范围见附录C及支撑材料《AI工具使用详情》。
...
图1—图18均由本地Python程序基于题给数据及本文结果文件生成。图表选题、指标口径、数据来源和图注由参赛队确定并人工复核；AI工具[14]仅辅助个别绘图脚本排错与版式检查。
...
截距不惩罚。岭回归用于缓解相关控制变量造成的估计不稳定[11]。在线性岭与二次岭各自的候选正则强度中，按训练期顺序设置3个扩展窗口滚动验证，二次岭选得$\lambda=0.1$，线性岭选得$\lambda=10$。最终模型只用训练期拟合，验证期用于模型比较与误差保护，回顾性测试期不参与选择。验证集绝对误差的90%分位数为11.78 kW，故每个候选同时报告点预测$\widehat P$与保守功耗$\widehat P+11.78$ kW；该常数保护不改变候选排序，但避免把点预测当作无误差预算。
...
本节程序的数学逻辑、约束定义、参数设置和结果验算由参赛队负责；AI工具[14]仅辅助个别报错定位和代码可读性检查。程序已在匿名支撑包内独立运行，关键结果由确定性核验脚本逐项比对。


### 5.4 10 mg/Nm³下的最佳采样可行方案


**表5　10 mg/Nm³中心情景的支持域内最佳采样可行解**


| 工况 | $U_1$/kV | $U_2$/kV | $U_3$/kV | $U_4$/kV | $T_1$/s | $T_2$/s | $T_3$/s | $T_4$/s | 峰值总排放/(mg/Nm³) | 功耗/kW |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
这些参数是支持域内的中心情景最佳采样可行解，不是设备安全设定。将其与同一功耗模型在各工况“历史中位控制点”上的预测比较，辅助节能幅度为9.80%～12.43%；该比较比“推荐点模型预测对历史实际均值”更同口径，但仍受排放情景假设影响。保守报告时，表中每个功耗均再加11.78 kW。现场应用必须继续受二次电流、火花率、联锁和设定变化速率约束。


![图11 两档限值的最优功耗](figures_paper/10_optimal_power.png)


**图11　四工况在10和5 mg/Nm³下的中心情景最佳采样功耗。** 更严格限值在全部工况下均提高功耗。
...
| 工况与限值 | $U_1$/kV | $U_2$/kV | $U_3$/kV | $U_4$/kV | $T_1$/s | $T_2$/s | $T_3$/s | $T_4$/s | 功耗/kW |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
![图12 最优电压配置](figures_paper/11_optimal_voltage.png)


**图12　两档排放限值下的四电场中心情景电压。** 严格约束主要抬升前两电场，同时$U_4$在部分工况承担明显补偿，故不作“只调前级”的绝对结论。


![图13 最优振打周期](figures_paper/12_optimal_rapping_period.png)


**图13　两档排放限值下的中心情景最佳采样振打周期。** 周期变化不像电压那样单向，而是在积灰损失与再飞扬峰值间重新平衡；前级短、后级长的顺序保持不变。
...
![图14 分工况增耗](figures_paper/18_condition_energy_penalty.png)
...
![图15 排放约束分解](figures_paper/13_emission_decomposition.png)


**图15　中心情景最佳采样解的连续排放基值与振打峰值增量。** 虚线是情景限值，所有柱均表示模型推演而非实测排放。
...
![图16 随机搜索收敛性](figures_paper/15_search_convergence.png)
...
![图17 增耗敏感性分布](figures_paper/16_sensitivity_distribution.png)


**图17　405组结构情景的增耗分布。** 出口缩放与振打相位会系统改变增耗，中心点约13.7%应与11.37%～14.31%的结构范围一起解释。


![图18 电场贡献权重消融](figures_paper/17_sensitivity_heatmap.png)
...
[1] 中华人民共和国环境保护部, 国家质量监督检验检疫总局. 水泥工业大气污染物排放标准：GB 4915—2013[S]. 北京：中国环境科学出版社, 2013.


[2] OGLESBY S Jr, NICHOLS G B. *A Manual of Electrostatic Precipitator Technology. Part I: Fundamentals*[R]. Report No. APTD-0610. Cincinnati: National Air Pollution Control Administration, 1970.


[3] SZABO M F, SHAH Y M, SCHLIESSER S P. *Inspection Manual for Evaluation of Electrostatic Precipitator Performance*[R]. Washington, DC: U.S. Environmental Protection Agency, 1981. https://nepis.epa.gov/Exe/ZyPURL.cgi?Dockey=9400034C.TXT [引用日期：2026-08-05].
...
[14] OPENAI. Codex[CP/OL]. 2026[2026-08-05]. https://openai.com/codex/.
...
完整可运行源程序随电子支撑材料提交：src/scenario_model.py（模型、优化与敏感性）、src/paper_figures.py（18幅正文图）、src/plot_utils.py（统一绘图样式）、src/integrity_check.py（确定性核验）。支撑数据、结果CSV/JSON、图注追踪表、回复审稿人与AI工具使用详情见附录C清单。


## 附录B 结果适用性声明


本文功耗模型、工况划分及数据质量统计直接来源于附件；出口浓度缩放、排放灰箱系数、振打峰值系数以及由此得到的5/10 mg/Nm³最佳采样可行参数和增耗均属于情景推演。若数据提供方确认原记录不存在小数点问题，或提供新的未封顶排放数据，则应撤销当前排放情景并重新标定，不得沿用本文情景数值。
...
**AI工具使用声明（提交前由参赛队核验确认）：** 本稿坚持参赛队人工主导。题意分析、模型构建、公式推导、参数设定、程序审阅、结果复算和论文定稿由参赛队承担；OpenAI Codex（使用日期2026-08-05）仅用于少量代码排错、图表版式检查、文字校对与格式核验。AI输出均经人工筛选和复核，未替代现场数据，也未把不可识别的排放关系改写为实证结论。详细用途和人工核验记录另见AI工具使用详情.pdf。
...
def select_optimum(
    ...
    row: dict[str, float | int | str] = {
        ...
        "conservative_power_kW": float(bank.iloc[idx]["predicted_power_kW"] + power_guard_kW),
        "power_guard_kW": float(power_guard_kW),
...
def make_figures(
    ...
    ax.set_xlabel("5 mg/Nm³ 相对 10 mg/Nm³ 的加权增耗 (%)")
    ax.set_ylabel("情景组合数")
...
def evaluate_power_models(
    ...
    metrics: dict[str, object] = {
        ...
        "conservative_reporting_rule": "point_prediction_plus_validation_absolute_error_q90",
...
def main() -> None:
    ...
    power_guard = float(metrics["validation_abs_error_q90_kW"])
    ...
    for seed in SEEDS:
        ...
        for _, profile in profiles.iterrows():
            ...
            preliminary = {limit: select_optimum(profile, base_bank, t_star, limit, power_guard_kW=power_guard) for limit in (10.0, 5.0)}
            additions = []
            ...
            if seed == SEED:
                ...
                for limit in (10.0, 5.0):
                    for fraction in (0.25, 0.50, 0.75, 1.00):
                        ...
                        result = select_optimum(profile, partial, t_star, limit, power_guard_kW=power_guard)
            ...
            for limit in (10.0, 5.0):
                final = select_optimum(profile, bank, t_star, limit, power_guard_kW=power_guard)
                final.update({
                    "seed": seed,
    ...
    for _, profile in profiles.iterrows():
        ...
        q4_rows.append({
            ...
            "conservative_power_10_kW": a["conservative_power_kW"],
            "conservative_power_5_kW": b["conservative_power_kW"],
    ...
    q4_summary = {
        ...
        "power_guard_kW": power_guard,
        "weighted_conservative_power_10_kW": weighted_p10+power_guard,
        "weighted_conservative_power_5_kW": weighted_p5+power_guard,
    }
    ...
    for eta in (0.8, 1.0, 1.2):
        for peak_scale in (0.9, 1.2, 1.5):
            for safety in (0.0, 0.08, 0.15):
                ...
                for _, profile in profiles.iterrows():
                    cluster = int(profile["condition_cluster"])
                    r10 = select_optimum(profile, banks[cluster], t_stars[cluster], 10.0, eta=eta, peak_scale=peak_scale, safety_index=safety, power_guard_kW=power_guard)
                    r5 = select_optimum(profile, banks[cluster], t_stars[cluster], 5.0, eta=eta, peak_scale=peak_scale, safety_index=safety, power_guard_kW=power_guard)
                ...
                for phase_scale in (0.65, 0.80, 1.00):
                    for tstar_exponent in (-0.30, -0.18, -0.10):
                        ...
                        for _, profile in profiles.iterrows():
                            ...
                            args = dict(outlet_scale=outlet_scale, alpha_weights=alpha_weights, peak_exponent=peak_exponent, phase_scale=phase_scale, power_guard_kW=power_guard)
    ...
    for alpha_name, alpha_weights in alpha_profiles.items():
        ...
        for _, profile in profiles.iterrows():
            cluster = int(profile["condition_cluster"])
            args = dict(alpha_weights=alpha_weights, power_guard_kW=power_guard)
    ...
    for _, profile in profiles.iterrows():
        ...
        dynamic_rows.append({
            "condition_cluster": cluster,
            "condition_name": profile["condition_name"],
            "strict_mode_entry_upper_bound_mgNm3": 4.5,
            "strict_mode_exit_upper_bound_mgNm3": 4.0,
            "minimum_dwell_min": 10,
...
from plot_utils import GRAY, LIGHT_GRAY, PALETTE, finish_axes, save_figure, set_paper_style
...
CAPTIONS = {
    ...
    "05_condition_profile_heatmap": "标准化工况画像揭示温度、浓度、流量、粉尘负荷与历史功耗之间的结构差异，为分工况控制提供依据。",
    "06_method_overview": "本文采用数据驱动功耗模型与工程灰箱排放模型协同的分层优化框架。实测数据负责功耗与工况辨识，排放约束仅在显式先验情景下推演。",
    "07_power_prediction": "滚动验证选择的二次岭回归优于均值与线性基线；回顾性测试R²为0.957、RMSE为14.76 kW。左图虚线表示理想预测，右图比较两时段RMSE。",
    "08_power_residuals": "回顾性测试残差均值为−10.78 kW，且在高功率区负偏差有所扩大；验证绝对误差90%分位数11.78 kW被用作保守功耗附加量。",
    "09_voltage_power_response": "在各工况典型状态下，模型给出的总功率随四电场电压同步提高而上升。曲线仅在各工况历史电压支持区间附近绘制。",
    "10_optimal_power": "更严格的5 mg/Nm³情景约束在四类工况下均要求更高功耗；中心情景工况加权平均功耗由1609.42 kW升至1829.79 kW。",
    ...
    "15_search_convergence": "五个独立随机种子与局部细化得到的最优功耗高度稳定；八个工况—限值组合的最大变异系数为0.181%，局部细化相对初始随机库最多改善0.986%。",
    "16_sensitivity_distribution": "405组结构情景全部在当前支持域内可行；5 mg/Nm³相对10 mg/Nm³的加权增耗范围为11.37%～14.31%，中位数为12.99%。箱线图展示出口缩放与振打相位的联合影响。",
    "17_sensitivity_heatmap": "场权重消融后，前两电场仍承担70.8%～73.7%的正向电压增量，说明前级优先并非完全由中心权重先验写入，但后级电压仍有不可忽略的补偿作用。",
    "18_condition_energy_penalty": "四类工况从10收紧至5 mg/Nm³的中心情景增耗为13.34%～14.11%，加权平均为13.69%；该点估计须与结构情景范围共同报告。",
}
...
TRACE_META = {
    ...
    "08_power_residuals": (str(DATA_FILE), "fig08_residuals + RidgePowerModel", "§5.2；图9", "存在−10.78 kW平均偏差和高功率区低估"),
    "09_voltage_power_response": (str(DATA_FILE), "fig09_voltage_response + RidgePowerModel", "§5.2；图10", "局部情景曲线，仅在历史电压附近解释"),
    "10_optimal_power": (str(RESULTS / "optimal_controls_central_scenario.csv"), "fig10_power", "§5.4；图11", "功耗来自数据模型；可行性来自未辨识灰箱情景"),
    "11_optimal_voltage": (str(RESULTS / "optimal_controls_central_scenario.csv"), "grouped_controls(U_COLS)", "§6；图12", "最优电压是情景试验起点，不是设备安全设定"),
    "12_optimal_rapping_period": (str(RESULTS / "optimal_controls_central_scenario.csv"), "grouped_controls(T_COLS)", "§6；图13", "无实际振打事件，周期结果依赖峰值先验"),
    "13_emission_decomposition": (str(RESULTS / "optimal_controls_central_scenario.csv"), "fig13_emission", "§7.2；图15", "全部排放值为情景推演，不是实测"),
    "14_rapping_peak_mechanism": (str(RESULTS / "condition_profiles.csv"), "fig14_peak", "§4.2；图4", "峰值指数1.35与尺度1.2属于工程先验"),
    "15_search_convergence": (str(RESULTS / "optimization_seed_stability.csv"), "fig15_seed_stability", "§8.1；图16", "随机种子稳定不等价于全局最优证明"),
    "16_sensitivity_distribution": (str(RESULTS / "structural_sensitivity.csv"), "fig16_structural_sensitivity", "§8.2；图17", "扫描范围仍由显式情景网格决定"),
    "17_sensitivity_heatmap": (str(RESULTS / "field_priority_ablation_summary.csv"), "fig17_field_ablation", "§8.3；图18", "消融仍依赖同一灰箱结构与支持域"),
    "18_condition_energy_penalty": (str(RESULTS / "question4_by_condition.csv"), "fig18_penalty", "§7.1；图14", "13.69%为单一种子中心情景点估计"),
}
...
MANUSCRIPT_FIGURE = {
    ...
    "08_power_residuals": 9,
    "09_voltage_power_response": 10,
    "10_optimal_power": 11,
    "11_optimal_voltage": 12,
    "12_optimal_rapping_period": 13,
    "18_condition_energy_penalty": 14,
    "13_emission_decomposition": 15,
    "15_search_convergence": 16,
    "16_sensitivity_distribution": 17,
    "17_sensitivity_heatmap": 18,
}
...
def fig02_timeseries(df: pd.DataFrame) -> None:
    ...
    for ax, (col, label, color) in zip(axes, fields):
...
def fig16_structural_sensitivity(structural: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("5相对10 mg/Nm³加权增耗 (%)")
    finish_axes(ax)
...
ROOT = SRC.parent
PROJECT_ROOT = ROOT.parent
DATA = PROJECT_ROOT / "数据处理" / "水泥电除尘器_数据处理四次返修包_交付链P1闭环_排放仍阻断" / "data" / "processed" / "full_timeseries_with_flags.csv"
RESULTS = ROOT / "results"
...
def main() -> None:
    ...
    check("power_guard_q90", close(guard, float(metrics["validation_abs_error_q90_kW"]), 1e-9), guard, metrics["validation_abs_error_q90_kW"])
    baselines = pd.read_csv(RESULTS / "power_model_baselines.csv")
    ...
    check("all_references_cited", cited == set(range(1, 15)), sorted(cited), list(range(1, 15)))
    check("reference_list_complete", listed == set(range(1, 15)), sorted(listed), list(range(1, 15)))
    ...
    check("table_caption_count", len(re.findall(r"^\*\*表\d+", clean_manuscript, flags=re.M)) == 7, len(re.findall(r"^\*\*表\d+", clean_manuscript, flags=re.M)), 7)
    check("manuscript_figure_count", len(re.findall(r"^!\[图", clean_manuscript, flags=re.M)) == 18, len(re.findall(r"^!\[图", clean_manuscript, flags=re.M)), 18)
    ...
    check("figure_manifest_count", len(manifest) == 18, len(manifest), 18)
